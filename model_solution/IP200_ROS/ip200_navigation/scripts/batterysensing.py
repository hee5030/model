#!/usr/bin/env python3
# 배터리 총량이 30.0v이고 전압분배회로를 거쳐서 1/6.1강하, A0핀으로 전압체크
import rospy
...
class BatteryStatusListener:
    ...
    def A0_callback(self, A0_msg):
        if self.prev_A0_msg is not None and A0_msg.data > self.prev_A0_msg:
            if self.voltage_msg >= 29.4:
                rospy.loginfo("voltage_msg: {}, percent_msg: {}, full charging".format(self.voltage_msg, self.percent_msg))
            else:
                rospy.loginfo("voltage_msg: {}, percent_msg: {}, charging".format(self.voltage_msg, self.percent_msg))
        else:
            rospy.loginfo("voltage_msg: {}, percent_msg: {}".format(self.voltage_msg, self.percent_msg))
    ...
    def voltage_callback(self, voltage_msg):
        ...
        if self.voltage_msg <= 26.5 and self.low_battery_flag == True:
            try:
                self.service_client()
